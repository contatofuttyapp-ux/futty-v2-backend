-- =====================================================================
-- Futty v2.0 — Migração 001: schema base
-- Tabelas: users, teams, team_members, games, votes + RLS + trigger.
-- Idempotente (IF NOT EXISTS / DROP POLICY IF EXISTS).
-- =====================================================================

create extension if not exists "pgcrypto";

-- TABELA: users (espelha auth.users; id igual ao de auth.users)
...
-- TABELA: team_members (role: admin | member)
...
-- TABELA: games (status: agendado | em_curso | terminado | cancelado)
...
-- TABELA: votes (nota 1-5; um voto por de_user/para_user/team — alterado em 004)
...
create policy "games_select_member" on public.games for select using (public.is_team_member(team_id));
create policy "games_insert_admin" on public.games for insert with check (public.is_team_admin(team_id));
create policy "games_update_admin" on public.games for update
  using (public.is_team_admin(team_id)) with check (public.is_team_admin(team_id));
create policy "games_delete_admin" on public.games for delete using (public.is_team_admin(team_id));
...
create policy "votes_select_member" on public.votes for select using (public.is_team_member(team_id));
...
create policy "votes_delete_self" on public.votes for delete using (de_user_id = auth.uid());

-- =====================================================================
-- TRIGGER: cria public.users automaticamente ao registar em auth.users
-- =====================================================================
